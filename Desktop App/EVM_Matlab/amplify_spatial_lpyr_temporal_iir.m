...
% Date: June 2012
%
function amplify_spatial_lpyr_temporal_iir(vidFile, outDir, ...
            alpha, lambda_c, r1, r2, chromAttenuation)
 
    [~,vidName] = fileparts(vidFile);
    
    outName = fullfile(outDir,[vidName '-amp.mp4']);
    
%     outName = fullfile(resultsDir,[vidName '-iir-r1-' num2str(r1)...
%         '-r2-' num2str(r2)...
%         '-alpha-' num2str(alpha) ...
%         '-lambda_c-' num2str(lambda_c) ...
%         '-chromAtn-' num2str(chromAttenuation) '.mp4']);
    ...
    vidOut = VideoWriter(outName,'MPEG-4');
    vidOut.FrameRate = fr;
